...
srun -u python3 main_haitam.py --data_dir=tasks/en/ --model_dir=logs/ --task {task} --vocab=data/vocab.txt --batch_size {batch_sizes} --learning_rate {learning_rate} --dropout_keep_prob {keep_rate} --num_epochs {max_epochs} --num_layers {num_layers} --embedding_size {embed_sizes} --kernel_size 16
    """.format(**search)


    with open('job_files/'+search['file_name'] + ".job", 'w') as f:
        f.write(job_content)
